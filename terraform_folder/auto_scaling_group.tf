resource "aws_autoscaling_group" "secure_data_vault_asg" {
  name = "secure_data_vault_asg"
  ...
  launch_template {
      id = aws_launch_template.secure_data_vault_lt.id    # I attach my launch template here using the launch template id
      version = "$Latest"
  }
...
resource "aws_autoscaling_policy" "cpu_tracking" {
  name = "cpu_tracking"
  autoscaling_group_name = aws_autoscaling_group.secure_data_vault_asg.name
...
output "asg_group_arn" {
    value = aws_autoscaling_group.secure_data_vault_asg.arn
}