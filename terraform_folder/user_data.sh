...
docker pull jackedu/secure_data_vault_repo:latest

# Run the Docker container
sudo docker run -d -p 80:80 --restart always --name secure_data_vault_container jackedu/secure_data_vault_repo:latest
